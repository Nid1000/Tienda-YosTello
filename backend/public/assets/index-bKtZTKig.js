(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))c(s);new MutationObserver(s=>{for(const n of s)if(n.type==="childList")for(const l of n.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&c(l)}).observe(document,{childList:!0,subtree:!0});function t(s){const n={};return s.integrity&&(n.integrity=s.integrity),s.referrerPolicy&&(n.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?n.credentials="include":s.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function c(s){if(s.ep)return;s.ep=!0;const n=t(s);fetch(s.href,n)}})();var q,z;const O=(z=(q=import.meta)==null?void 0:q.env)==null?void 0:z.VITE_API_BASE_URL;var E;const I=(E=document.querySelector('meta[name="api-base-url"]'))==null?void 0:E.getAttribute("content"),P=(O||I||"http://127.0.0.1:8000/api").replace(/\/$/,""),r=P.replace(/\/api$/,""),x=document.querySelector("#app");x.innerHTML=`
        ...
        <section id="account-panel" class="account-panel hidden">
            ...
            <div class="account-card-wrap">
                <div class="account-card">
                    ...
                    <a href="${r}/dashboard" target="_blank" rel="noreferrer">Ajustes de la cuenta</a>
                    <a href="${r}/dashboard" target="_blank" rel="noreferrer">Mi libreta de direcciones</a>
                    <a href="${r}/checkout" target="_blank" rel="noreferrer">Tarjeta de credito</a>
                    <a href="${r}/productos" target="_blank" rel="noreferrer">Guardados</a>
        ...
        <main>
            ...
            <section class="promo-runway" id="active-promotions">
                <div class="promo-runway-copy">
                    ...
                    <h2>Promociones con mirada de pasarela</h2>
                    <p>Gestionadas desde el panel administrador y conectadas al backend para destacar lanzamientos, codigos y capsulas.</p>
...
`;const k=document.querySelector("#overlay"),B=document.querySelector("#contact-visual"),H=document.querySelector("#menu-visual"),N=document.querySelector("#account-visual"),f=document.querySelector("#category-grid"),S=document.querySelector("#backend-stats"),g=document.querySelector("#featured-grid"),_=document.querySelector("#promotions-grid"),v=document.querySelector("#latest-grid"),D=document.querySelector("#bag-count"),y=document.querySelector("#bag-content"),C=document.querySelector("#bag-heading"),A=document.querySelector("#search-input"),$=document.querySelector("#search-results"),m=document.querySelector("#product-detail"),j=document.querySelector("#cart-dialog-body"),o={products:[],cart:JSON.parse(localStorage.getItem("yotello-cart")||"[]"),selectedSize:"M / 40",activeProductId:null};o.cart=(Array.isArray(o.cart)?o.cart:[]).map(a=>({...a,size:(a==null?void 0:a.size)||"M / 40"}));const i=a=>new Intl.NumberFormat("es-CO",{style:"currency",currency:"COP",maximumFractionDigits:0}).format(a||0),M=()=>localStorage.setItem("yotello-cart",JSON.stringify(o.cart)),R=a=>{o.cart=o.cart.filter(e=>e.id!==a),M(),p()},Y=(a,e)=>{if(!o.cart.length){window.open(e,"_blank","noopener,noreferrer");return}const t=document.createElement("form");t.method="POST",t.action=`${r}/bridge/cart`,t.target="_blank",t.style.display="none";const c=document.createElement("input");c.type="hidden",c.name="target",c.value=a;const s=document.createElement("input");s.type="hidden",s.name="cart",s.value=JSON.stringify(o.cart.map(n=>({id:n.id,qty:n.qty,size:n.size||o.selectedSize}))),t.appendChild(c),t.appendChild(s),document.body.appendChild(t),t.submit(),t.remove()},u=a=>{var e;document.querySelectorAll(".contact-panel, .account-panel, .menu-panel, .search-panel, .product-panel, .bag-view, .size-panel, .cart-dialog").forEach(t=>t.classList.add("hidden")),k.classList.remove("hidden"),(e=document.querySelector(`#${a}-panel, #${a}`))==null||e.classList.remove("hidden")},h=()=>{document.querySelectorAll(".contact-panel, .account-panel, .menu-panel, .search-panel, .product-panel, .bag-view, .size-panel, .cart-dialog").forEach(a=>a.classList.add("hidden")),k.classList.add("hidden")},w=a=>{var c;const e=o.products.find(s=>s.id===a);if(!e)return;o.activeProductId=a;const t=o.products.filter(s=>s.category===e.category).slice(0,4);m.innerHTML=`
    ...
    `,d(m),b(m),(c=m.querySelector("[data-open-size]"))==null||c.addEventListener("click",()=>u("size")),u("product")},F=a=>{j.innerHTML=`
    ...
    `,u("cart-dialog")},G=a=>{const e=o.products.find(c=>c.id===a);if(!e)return;const t=o.cart.find(c=>c.id===a);t?(t.qty+=1,t.size=t.size||o.selectedSize):o.cart.push({id:e.id,name:e.name,category:e.category,image:e.image,final_price:e.final_price??e.price,qty:1,size:o.selectedSize}),M(),p(),F(e)},p=()=>{const a=o.cart.reduce((e,t)=>e+t.qty,0);if(D.textContent=String(a),o.cart.length===0)C.textContent="Su bolsa de compra esta actualmente vacia",y.innerHTML=`
        ...
        `;else{const e=o.cart.reduce((t,c)=>t+c.final_price*c.qty,0);C.textContent=`Tiene ${a} articulo${a===1?"":"s"} en su bolsa`,y.innerHTML=`
                ...
                <div class="bag-item-list">
                    ${o.cart.map(t=>`
                        <article class="bag-line">
                            ...
                            <strong>${i(t.final_price*t.qty)}</strong>
                        </article>
                    `).join("")}
                </div>
...
`,b=(a=document)=>{a.querySelectorAll("[data-add-id]").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),G(Number(e.dataset.addId))})})},d=(a=document)=>{a.querySelectorAll("[data-open-product]").forEach(e=>{e.addEventListener("click",()=>w(Number(e.dataset.openProduct)))})},L=(a="")=>{const e=a.trim().toLowerCase(),t=e?o.products.filter(c=>[c.name,c.category,c.description].some(s=>s.toLowerCase().includes(e))):o.products.slice(0,6);document.querySelector("#search-meta").textContent=`${t.length} resultado${t.length===1?"":"s"} encontrados`,$.innerHTML=t.map(X).join(""),d($),b($)};document.querySelectorAll("[data-panel-target]").forEach(a=>{a.addEventListener("click",()=>{a.dataset.panelTarget==="bag"&&p(),u(a.dataset.panelTarget)})});document.querySelectorAll("[data-close-panel]").forEach(a=>a.addEventListener("click",h));var T;(T=document.querySelector("[data-open-bag]"))==null||T.addEventListener("click",()=>{p(),u("bag")});k.addEventListener("click",h);A.addEventListener("input",a=>L(a.target.value));document.querySelectorAll("[data-search-term]").forEach(a=>{a.addEventListener("click",()=>{A.value=a.dataset.searchTerm,L(a.dataset.searchTerm)})});document.addEventListener("click",a=>{if(a.target.closest("a[data-go-home]")){a.preventDefault(),o.activeProductId=null,h(),window.scrollTo(0,0);return}const t=a.target.closest("button[data-remove-id]");if(t){a.preventDefault(),a.stopPropagation(),R(Number(t.dataset.removeId));return}const c=a.target.closest("a[data-bridge-target]");c&&(a.preventDefault(),Y(c.dataset.bridgeTarget,c.href))});document.querySelectorAll(".size-option").forEach(a=>{a.addEventListener("click",()=>{document.querySelectorAll(".size-option").forEach(e=>e.classList.remove("active-size")),a.classList.add("active-size"),o.selectedSize=a.textContent.trim(),o.activeProductId?w(o.activeProductId):h()})});fetch(`${P}/storefront/overview`).then(async a=>{if(!a.ok)throw new Error("No se pudo cargar la portada desde el backend.");return a.json()}).then(a=>{var t;o.products=[...a.featuredProducts,...a.latestProducts].filter((c,s,n)=>n.findIndex(l=>l.id===c.id)===s);const e=a.featuredProducts[0]||a.latestProducts[0];if(e){const c=`
                linear-gradient(180deg, rgba(35, 24, 18, 0.24) 0%, rgba(18, 12, 8, 0.74) 100%),
                url('${e.image}')
            `;[B,H,N].forEach(s=>{s.style.backgroundImage=c})}f.innerHTML=(a.featuredProducts.length?a.featuredProducts:a.latestProducts).slice(0,4).map(V).join(""),S.innerHTML=`
        ...
        `,g.innerHTML=a.featuredProducts.map(J).join(""),_.innerHTML=(t=a.promotions)!=null&&t.length?a.promotions.map(U).join(""):'<div class="product-loading">Crea promociones desde el administrador para destacarlas aqui.</div>',v.innerHTML=a.latestProducts.map(W).join(""),d(f),d(g),d(v),b(g),b(v),L(),p()}).catch(a=>{f.innerHTML=`<div class="category-placeholder">${a.message}</div>`,S.innerHTML=`<div class="stat-card muted-card">${a.message}</div>`,g.innerHTML=`<div class="product-loading">${a.message}</div>`,_.innerHTML=`<div class="product-loading">${a.message}</div>`,v.innerHTML=`<div class="product-loading">${a.message}</div>`});
