(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))c(s);new MutationObserver(s=>{for(const n of s)if(n.type==="childList")for(const l of n.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&c(l)}).observe(document,{childList:!0,subtree:!0});function t(s){const n={};return s.integrity&&(n.integrity=s.integrity),s.referrerPolicy&&(n.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?n.credentials="include":s.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function c(s){if(s.ep)return;s.ep=!0;const n=t(s);fetch(s.href,n)}})();var z,E;const j=(E=(z=import.meta)==null?void 0:z.env)==null?void 0:E.VITE_API_BASE_URL;var T;const R=(T=document.querySelector('meta[name="api-base-url"]'))==null?void 0:T.getAttribute("content"),B=(j||R||"http://127.0.0.1:8000/api").replace(/\/$/,""),r=B.replace(/\/api$/,"");var P,A;const b=(A=(P=document.querySelector('meta[name="auth-user-initial"]'))==null?void 0:P.getAttribute("content"))==null?void 0:A.trim();var M,w;const Y=(w=(M=document.querySelector('meta[name="auth-user-name"]'))==null?void 0:M.getAttribute("content"))==null?void 0:w.trim();var O;const F=((O=document.querySelector('meta[name="logout-url"]'))==null?void 0:O.getAttribute("content"))||`${r}/logout`;var I;const V=((I=document.querySelector('meta[name="csrf-token"]'))==null?void 0:I.getAttribute("content"))||"",G=document.querySelector("#app"),U=b?`<button class="topbar-action account-action" data-panel-target="account"><span class="account-initial-badge">${b.slice(0,1).toUpperCase()}</span><span>Cuenta</span></button>`:'<button class="topbar-action" data-panel-target="account">Cuenta</button>',J=b?`
        <div class="account-user-card">
            <span class="account-initial-large">${b.slice(0,1).toUpperCase()}</span>
            <div>
                <strong>${Y||"Mi cuenta"}</strong>
                <small>Sesion iniciada</small>
            </div>
        </div>
        <a href="${r}/pedidos">Mis pedidos</a>
        <a href="${r}/dashboard">Mi cuenta</a>
        <form class="account-logout-form" method="POST" action="${F}">
            <input type="hidden" name="_token" value="${V}">
...
`;const S=document.querySelector("#overlay"),W=document.querySelector("#contact-visual"),X=document.querySelector("#menu-visual"),Z=document.querySelector("#account-visual"),y=document.querySelector("#category-grid"),C=document.querySelector("#backend-stats"),g=document.querySelector("#featured-grid"),q=document.querySelector("#promotions-grid"),m=document.querySelector("#latest-grid"),K=document.querySelector("#bag-count"),$=document.querySelector("#bag-content"),_=document.querySelector("#bag-heading"),H=document.querySelector("#search-input"),k=document.querySelector("#search-results"),v=document.querySelector("#product-detail"),Q=document.querySelector("#cart-dialog-body"),o={products:[],cart:JSON.parse(localStorage.getItem("yotello-cart")||"[]"),selectedSize:"M / 40",activeProductId:null};o.cart=(Array.isArray(o.cart)?o.cart:[]).map(a=>({...a,size:(a==null?void 0:a.size)||"M / 40"}));const i=a=>new Intl.NumberFormat("es-CO",{style:"currency",currency:"COP",maximumFractionDigits:0}).format(a||0),N=()=>localStorage.setItem("yotello-cart",JSON.stringify(o.cart)),aa=a=>{o.cart=o.cart.filter(e=>e.id!==a),N(),p()},ea=(a,e)=>{if(!o.cart.length){window.open(e,"_blank","noopener,noreferrer");return}const t=document.createElement("form");t.method="POST",t.action=`${r}/bridge/cart`,t.target="_blank",t.style.display="none";const c=document.createElement("input");c.type="hidden",c.name="target",c.value=a;const s=document.createElement("input");s.type="hidden",s.name="cart",s.value=JSON.stringify(o.cart.map(n=>({id:n.id,qty:n.qty,size:n.size||o.selectedSize}))),t.appendChild(c),t.appendChild(s),document.body.appendChild(t),t.submit(),t.remove()},u=a=>{var e;document.querySelectorAll(".contact-panel, .account-panel, .menu-panel, .search-panel, .product-panel, .bag-view, .size-panel, .cart-dialog").forEach(t=>t.classList.add("hidden")),S.classList.remove("hidden"),(e=document.querySelector(`#${a}-panel, #${a}`))==null||e.classList.remove("hidden")},f=()=>{document.querySelectorAll(".contact-panel, .account-panel, .menu-panel, .search-panel, .product-panel, .bag-view, .size-panel, .cart-dialog").forEach(a=>a.classList.add("hidden")),S.classList.add("hidden")},D=a=>{var c;const e=o.products.find(s=>s.id===a);if(!e)return;o.activeProductId=a;const t=o.products.filter(s=>s.category===e.category).slice(0,4);v.innerHTML=`
    ...
    `,d(v),h(v),(c=v.querySelector("[data-open-size]"))==null||c.addEventListener("click",()=>u("size")),u("product")},ta=a=>{Q.innerHTML=`
    ...
    `,u("cart-dialog")},sa=a=>{const e=o.products.find(c=>c.id===a);if(!e)return;const t=o.cart.find(c=>c.id===a);t?(t.qty+=1,t.size=t.size||o.selectedSize):o.cart.push({id:e.id,name:e.name,category:e.category,image:e.image,final_price:e.final_price??e.price,qty:1,size:o.selectedSize}),N(),p(),ta(e)},p=()=>{const a=o.cart.reduce((e,t)=>e+t.qty,0);if(K.textContent=String(a),o.cart.length===0)_.textContent="Su bolsa de compra esta actualmente vacia",$.innerHTML=`
...
`,h=(a=document)=>{a.querySelectorAll("[data-add-id]").forEach(e=>{e.addEventListener("click",t=>{t.stopPropagation(),sa(Number(e.dataset.addId))})})},d=(a=document)=>{a.querySelectorAll("[data-open-product]").forEach(e=>{e.addEventListener("click",()=>D(Number(e.dataset.openProduct)))})},L=(a="")=>{const e=a.trim().toLowerCase(),t=e?o.products.filter(c=>[c.name,c.category,c.description].some(s=>s.toLowerCase().includes(e))):o.products.slice(0,6);document.querySelector("#search-meta").textContent=`${t.length} resultado${t.length===1?"":"s"} encontrados`,k.innerHTML=t.map(ia).join(""),d(k),h(k)};document.querySelectorAll("[data-panel-target]").forEach(a=>{a.addEventListener("click",()=>{a.dataset.panelTarget==="bag"&&p(),u(a.dataset.panelTarget)})});document.querySelectorAll("[data-close-panel]").forEach(a=>a.addEventListener("click",f));var x;(x=document.querySelector("[data-open-bag]"))==null||x.addEventListener("click",()=>{p(),u("bag")});S.addEventListener("click",f);H.addEventListener("input",a=>L(a.target.value));document.querySelectorAll("[data-search-term]").forEach(a=>{a.addEventListener("click",()=>{H.value=a.dataset.searchTerm,L(a.dataset.searchTerm)})});document.addEventListener("click",a=>{if(a.target.closest("a[data-go-home]")){a.preventDefault(),o.activeProductId=null,f(),window.scrollTo(0,0);return}const t=a.target.closest("button[data-remove-id]");if(t){a.preventDefault(),a.stopPropagation(),aa(Number(t.dataset.removeId));return}const c=a.target.closest("a[data-bridge-target]");c&&(a.preventDefault(),ea(c.dataset.bridgeTarget,c.href))});document.querySelectorAll(".size-option").forEach(a=>{a.addEventListener("click",()=>{document.querySelectorAll(".size-option").forEach(e=>e.classList.remove("active-size")),a.classList.add("active-size"),o.selectedSize=a.textContent.trim(),o.activeProductId?D(o.activeProductId):f()})});fetch(`${B}/storefront/overview`).then(async a=>{if(!a.ok)throw new Error("No se pudo cargar la portada desde el backend.");return a.json()}).then(a=>{var t;o.products=[...a.featuredProducts,...a.latestProducts].filter((c,s,n)=>n.findIndex(l=>l.id===c.id)===s);const e=a.featuredProducts[0]||a.latestProducts[0];if(e){const c=`
        ...
        `,g.innerHTML=a.featuredProducts.map(oa).join(""),q.innerHTML=(t=a.promotions)!=null&&t.length?a.promotions.map(na).join(""):'<div class="product-loading">Crea promociones desde el administrador para destacarlas aqui.</div>',m.innerHTML=a.latestProducts.map(ca).join(""),d(y),d(g),d(m),h(g),h(m),L(),p()}).catch(a=>{y.innerHTML=`<div class="category-placeholder">${a.message}</div>`,C.innerHTML=`<div class="stat-card muted-card">${a.message}</div>`,g.innerHTML=`<div class="product-loading">${a.message}</div>`,q.innerHTML=`<div class="product-loading">${a.message}</div>`,m.innerHTML=`<div class="product-loading">${a.message}</div>`});
