  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (dniForm && ubigeoScript) {
      ...
      if (departmentSelect && provinceSelect && districtSelect) {
        ...
        const syncProvinces = (departmentName, selectedProvince = '', selectedDistrict = '') => {
          ...
          const provinces = (department && department.provinces) || [];
          ...
          syncDistricts((province && province.districts) || [], selectedDistrict);
        };
        ...
        provinceSelect.addEventListener('change', () => {
          ...
          const province = ((department && department.provinces) || []).find(
            (item) => normalizeText(item.name) === normalizeText(provinceSelect.value)
          );

          syncDistricts((province && province.districts) || []);
        });
      }
    }
    ...
    trigger?.addEventListener('click', async () => {
      ...
      try {
        ...
        const data = await postJson(requestUrl, payload);
        ...
        feedback.textContent = error?.message || `No se pudo consultar el ${documentType.value}.`;
      }
    });
  });
})();
